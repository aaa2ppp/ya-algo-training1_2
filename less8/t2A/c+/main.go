...
func solve(graph [][]int) int {
	// тупо считаем области связности

	var dfs func(node int)

	visited := make([]bool, len(graph))
	var buf []int // for debug only
	...
	for node, n := 1, len(graph); node < n; node++ {
		if !visited[node] {
			count++
			buf = buf[:0]
			dfs(node)
			if debugEnable {
				log.Printf("%d: %v", count, buf)
			}
		}
	}
...
func run(in io.Reader, out io.Writer) {
	...
	for i := 1; i <= n; i++ {
		...
		// т.к. для решения задачи мы будем считать области связности,
		// готовим НЕорентированный граф
		graph[a] = append(graph[a], i)
		graph[i] = append(graph[i], a)
	}
