...
func NewTreeNode[T cmp.Ordered](v T) *TreeNode[T] {
	return &TreeNode[T]{
		Value:  v,
		height: 1,
		isAVL:  true,
...
// Add добавлет узел в дерево
func (p *TreeNode[T]) Add(v T) {
	if v < p.Value {
		if p.Left == nil {
...
func (p *TreeNode[T]) update() {
	lh := p.Left.Height()
	rh := p.Right.Height()
	p.height = max(lh, rh) + 1
	p.isAVL = p.Left.IsAVL() && p.Right.IsAVL() && abs(lh-rh) <= 1
}

func (p *TreeNode[T]) Max() T {
	if p.Right != nil {
		return p.Right.Max()
	}
	return p.Value
...
func (p *TreeNode[T]) SecondMax() (T, bool) {
	if p.Right != nil {
		v, ok := p.Right.SecondMax()
		if ok {
			return v, true
		}
		return p.Value, true
	...
	// it maximum
	if p.Left != nil {
		return p.Left.Max(), true
	}
	return *new(T), false
}

func (p *TreeNode[T]) Items(buf []T) []T {
	if p.Left != nil {
		buf = p.Left.Items(buf)
	}
	buf = append(buf, p.Value)
	if p.Right != nil {
		buf = p.Right.Items(buf)
	}
	return buf
}

func (p *TreeNode[T]) Leaves(buf []T) []T {
	if p.Left == nil && p.Right == nil {
		return append(buf, p.Value)
	}
	if p.Left != nil {
		buf = p.Left.Leaves(buf)
	}
	if p.Right != nil {
		buf = p.Right.Leaves(buf)
	}
	return buf
}

func (p *TreeNode[T]) Forks(buf []T) []T {
	if p.Left != nil {
		buf = p.Left.Forks(buf)
	}
	if p.Left != nil && p.Right != nil {
		buf = append(buf, p.Value)
	}
	if p.Right != nil {
		buf = p.Right.Forks(buf)
	}
	return buf
}

func (p *TreeNode[T]) Branches(buf []T) []T {
	if p.Left != nil {
		buf = p.Left.Branches(buf)
	}
	if (p.Left != nil || p.Right != nil) && !(p.Left != nil && p.Right != nil) {
		buf = append(buf, p.Value)
	}
	if p.Right != nil {
		buf = p.Right.Branches(buf)
	}
	return buf
}
