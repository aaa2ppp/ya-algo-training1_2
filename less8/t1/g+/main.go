...
// Add добавлет узел в дерево и возвращает его глубину
func (p *TreeNode[T]) Add(v T) int {
	if v == p.Value {
		return 1
	}
	if v < p.Value {
		if p.Left != nil {
			return p.Left.Add(v) + 1
		}
		p.Left = &TreeNode[T]{Value: v}
	} else {
		if p.Right != nil {
			return p.Right.Add(v) + 1
		}
...
func run(in io.Reader, out io.Writer) {
	...
	if root != nil {
		items = root.Branches(items)
	}

	writeInts(bw, items, writeOpts{sep: '\n', end: '\n'})
}
