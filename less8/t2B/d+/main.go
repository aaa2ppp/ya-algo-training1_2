...
// solve возвращает количество узлов в самом длинном пути в графе.
// Функция ожидает, что граф не имеет циклов (по условию это деререво).
func solve(graph [][]int) int {
	maximum := -1

	var dfs func(node, prev int, h int)

	dfs = func(node, prev int, h int) {
	...
	for node := range graph {
		// что бы н делать лишних обходов, выбираем в качестве корня лист
		if len(graph[node]) == 1 { // (*) это работает и для 0 и 1-indexing
			dfs(node, -1, 0)
		}
	}
